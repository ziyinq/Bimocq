...
template<class T>
void RBGS2D(const FixedSparseMatrix<T> &A,
	const vector<T> &b,
	vector<T> &x,
...
template<class T>
void amgVCycle2D(vector<FixedSparseMatrix<T> *> &A_L,
	vector<FixedSparseMatrix<T> *> &R_L,
	vector<FixedSparseMatrix<T> *> &P_L,
	vector<Vec2i>                  &S_L,
	vector<T>                      &x,
	const vector<T>                &b)
{
	...
	for (int i = 1;i<total_level;i++)
	{
		int unknowns = S_L[i].v[0] * S_L[i].v[1];
		x_L[i].resize(unknowns);
		x_L[i].assign(x_L[i].size(), 0);
		b_L[i].resize(unknowns);
		b_L[i].assign(b_L[i].size(), 0);
	...
	for (int i = 0;i<total_level - 1;i++)
	{
		RBGS2D(*(A_L[i]), b_L[i], x_L[i], S_L[i].v[0], S_L[i].v[1], 4);
		restriction(*(R_L[i]), *(A_L[i]), x_L[i], b_L[i], b_L[i + 1]);
	}
	int i = total_level - 1;
	RBGS2D(*(A_L[i]), b_L[i], x_L[i], S_L[i].v[0], S_L[i].v[1], 500);
	for (int i = total_level - 2;i >= 0;i--)
	{
		prolongatoin(*(P_L[i]), x_L[i + 1], x_L[i]);
		RBGS2D(*(A_L[i]), b_L[i], x_L[i], S_L[i].v[0], S_L[i].v[1], 4);
...
template<class T>
void amgPrecond2D(vector<FixedSparseMatrix<T> *> &A_L,
	vector<FixedSparseMatrix<T> *> &R_L,
	vector<FixedSparseMatrix<T> *> &P_L,
	vector<Vec2i>                  &S_L,
	vector<T>                      &x,
	const vector<T>                &b)
{
	x.resize(b.size());
	x.assign(x.size(), 0);
	amgVCycle2D(A_L, R_L, P_L, S_L, x, b);
}
template<class T>
						   ...
						   bool PURE_NEUMANN)
{
	...
	double tol = std::max(tolerance_factor*residual_out, tolerance_factor);
	if(PURE_NEUMANN) r = v;
